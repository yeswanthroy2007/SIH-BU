...
// Enhanced header mapping for different CSV formats
const HEADER_MAPPING = {
  ...
  'Contact': 'special_notes',
  'State': 'description'
};

// Clean and standardize data
function cleanValue(value) {
  if (!value || value === 'Not documented' || value === 'nan' || value === '' || value === 'null') {
    return null;
  }
...
function mapHeaders(oldHeaders) {
  return oldHeaders.map(header => {
    ...
    return HEADER_MAPPING[cleanHeader] || cleanHeader.toLowerCase().replace(/\s+/g, '_');
  });
}
...
function processCSVFile(filePath, stateName, stateCode) {
  ...
  try {
    ...
    console.log(`  📋 Headers: ${oldHeaders.slice(0, 5).join(', ')}...`);
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      // Try to extract place name from first column if not mapped
      if (!place.place_name && values[0]) {
        place.place_name = cleanValue(values[0]);
      }
      
      // Try to extract category from second column if not mapped
      if (!place.category && values[1]) {
        place.category = cleanValue(values[1]);
      }
